...
export class QuizService {
  ...
  async updateQuizQuestion(
    ...
    try {
      const allowed: any = {};
      if (data.question !== undefined) allowed.question = data.question;
      if (data.answer !== undefined) allowed.answer = data.answer;
      if (data.distractor1 !== undefined) allowed.distractor1 = data.distractor1;
      if (data.distractor2 !== undefined) allowed.distractor2 = data.distractor2;
      if (data.distractor3 !== undefined) allowed.distractor3 = data.distractor3;

      const updated = await this.quizQuestionModel
  ...
  async deleteQuizQuestion(
    ...
    try {
      const deleted = await this.quizQuestionModel.findByIdAndDelete(id).exec();
      if (!deleted) {
        throw new NotFoundException('Question not found');
      }
      // Cũng xóa reference ra khỏi mọi quiz có chứa question này
      ...
      if (error instanceof NotFoundException) throw error;
      throw new BadRequestException("Couldn't delete question.");
    }
  }
  ...
  async updateQuiz(
    ...
    try {
      ...
      if (data.timeLimit !== undefined)
        allowed.timeLimit = Number(data.timeLimit);
      ...
      if (error instanceof NotFoundException) throw error;
      throw new BadRequestException(error.message || "Couldn't update quiz.");
    }
  }
  ...
  async deleteOneQuiz(
    ...
    try {
      ...
      if (error instanceof NotFoundException) throw error;
      throw new BadRequestException("Couldn't delete quiz.");
    }
  }
