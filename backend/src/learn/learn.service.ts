...
export class LearnService {
  ...
  async findWordByName(word: string): Promise<any> {
    const found = await this.wordModel
      .findOne({ word: word.toLowerCase().trim() })
      .lean()
      .exec();
    return found || null;
  }

  async getWordsByTitle(lessonTitle: string): Promise<any[]> {
    ...
    return words.sort((a, b) => {
      ...
      if (lvl !== 0) return lvl;
      return (a.wordNumber || 0) - (b.wordNumber || 0);
    });
  }
  ...
  async updateLesson(
    ...
    try {
      ...
      throw new BadRequestException(error.message || "Couldn't update lesson.");
    }
  }
  ...
  async deleteOneLesson(
    ...
    try {
      ...
      if (error instanceof NotFoundException) throw error;
      throw new BadRequestException("Couldn't delete lesson.");
    }
  }
  ...
  async updateWord(
    ...
    try {
      ...
      if (error instanceof NotFoundException) throw error;
      throw new BadRequestException(error.message || "Couldn't update word.");
    }
  }
  ...
  async deleteOneWord(
    ...
    try {
      const deleted = await this.wordModel.findByIdAndDelete(id).exec();
      if (!deleted) {
        throw new NotFoundException('Word not found');
      }
      ...
      if (error instanceof NotFoundException) throw error;
      throw new BadRequestException("Couldn't delete word.");
    }
  }
  ...
  async startLearning(
    ...
    try {
      ...
      // All lessons unlocked from the start — no sequential gating.
      await Promise.all(
        lessons.map((lesson) =>
